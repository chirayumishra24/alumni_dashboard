...
async function main() {
  ...
  await prisma.widgetSpeak.deleteMany({});
  ...
  // Seed CCHS Alumni (15 profiles, all verified)
  const cchsAlumni = [
  ...
  // Seed CCWS Alumni (15 profiles, all verified)
  const ccwsAlumni = [
  ...
  // Seed Unverified/Pending Registrations (for verification queue testing)
  const pendingAlumni = [
  ...
  for (const data of pendingAlumni) {
    const user = await prisma.user.create({
      data: {
        email: data.email,
        name: data.name,
  ...
  await prisma.mentorship.create({
    data: {
      ...
      alumniId: cchsProfiles[0].id, // Priya Singh
      status: 'PENDING',
  ...
  await prisma.mentorship.create({
    data: {
      ...
      alumniId: ccwsProfiles[0].id, // Vikram Malhotra
      status: 'ACCEPTED',
  ...
  await prisma.event.createMany({
    data: [
      ...
      },
      {
        ...
        eventDate: new Date(Date.now() + 1000 * 60 * 60 * 24 * 12),
  ...
  console.log('Seeded 30 Verified Alumni (15 CCHS / 15 CCWS), 2 Pending/Unverified Registrations, Mentees, and Events.');
}
