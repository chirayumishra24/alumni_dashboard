      ...
      <div className={`w-full max-w-md p-8 rounded-[2rem] glass-panel shadow-2xl relative transition-all duration-300 ${shake ? "animate-shake" : ""}`}>
        ...
        <form onSubmit={handleLogin} className="space-y-5">
          <div className="space-y-2">
            <label className="text-[10px] font-bold text-slate-405 uppercase tracking-wider block">Admin Password</label>
            <div className="relative">
              ...
              <input
                ...
                className="w-full glass-input rounded-2xl pl-11 pr-12 py-3.5 text-sm text-slate-800 placeholder-slate-400 focus:outline-none font-medium"
              />
              <button
                type="button"
          ...
          {error && (
            <div className="flex items-center gap-2 text-xs font-semibold text-rose-800 bg-rose-50/50 border border-rose-100/50 p-3.5 rounded-2xl animate-fade-in">
          ...
          <button
            type="submit"
            disabled={loading}
            className="w-full py-3.5 rounded-2xl bg-gradient-to-r from-violet-600 to-indigo-650 hover:shadow-lg hover:shadow-indigo-500/15 text-xs font-bold text-white transition-all hover:scale-[1.01] flex items-center justify-center gap-2 disabled:opacity-75 disabled:hover:scale-100"
          >
            {loading ? (
